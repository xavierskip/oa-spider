...
JZWJW_CAPTCHA_URL = 'http://219.140.163.109:8080/oa/login/image.jsp'
...
def xprojection(img, box=()):
    """pixels counter on vertical 
        img: Image object with mode "L"
        box: (left, upper, right, lower) region of the img
    return: list
    """
    if not box:
...
def lonely_pixel(pixs, pos):
    """
    pixs: image.load() and image.mode == "L"
    pos: (x,y)
    """
    x,y = pos
    xy = [-1,0,1]
...
def img2str(img):
    """
    img: Image object and img.mode == "L"
    return string
    """
    xsize, ysize = img.size
    content = ''
